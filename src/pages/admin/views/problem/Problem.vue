...
<script>
  ...
  export default {
    ...
    data () {
      return {
        rules: {
          _id: {required: true, message: 'Display ID를 입력해 주세요.', trigger: 'blur'},
          title: {required: true, message: '제목을 입력해 주세요.', trigger: 'blur'},
          input_description: {required: true, message: '입력 설명을 적어주세요.', trigger: 'blur'},
          output_description: {required: true, message: '출력 설명을 적어주세요.', trigger: 'blur'}
        },
    ...
    methods: {
      ...
      submit () {
        if (!this.problem.samples.length) {
          this.$error('예제를 적어주세요.')
          return
        }
        for (let sample of this.problem.samples) {
          if (!sample.input || !sample.output) {
            this.$error('예제 입력과 출력을 적어주세요.')
            return
          }
        }
        if (!this.problem.tags.length) {
          this.error.tags = '적어도 한 개 이상의 태그를 적어주세요.'
          this.$error(this.error.tags)
          return
        }
        if (this.problem.spj) {
          if (!this.problem.spj_code) {
            this.error.spj = '체커를 작성해 주세요.'
            this.$error(this.error.spj)
          } else if (!this.problem.spj_compile_ok) {
            this.error.spj = '체커가 컴파일되지 않았어요.'
          }
          if (this.error.spj) {
            this.$error(this.error.spj)
            return
        ...
        if (!this.problem.languages.length) {
          this.error.languages = '적어도 한 개 이상의 언어를 선택해 주세요.'
          this.$error(this.error.languages)
          return
        }
        if (!this.testCaseUploaded) {
          this.error.testCase = '테스트 케이스를 업로드 해 주세요.'
