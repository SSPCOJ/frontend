        ...
        <Card :padding="20" class="flex-child">
          ...
          <div slot="extra">
            ...
            <Button v-else
                    type="warning"
                    size="small"
                    @click="deleteSession(session.session_key)">종료
            </Button>
          </div>
      ...
      <Form>
        ...
        <template v-if="!loadingQRcode">
          <FormItem style="width: 250px">
            <Input v-model="formTwoFactor.code" placeholder="이중 인증 코드"/>
          </FormItem>
          <Button type="primary"
                  :loading="loadingBtn"
                  @click="updateTFA(false)"
                  v-if="!TFAOpened">이중 인증 활성화
          </Button>
          <Button type="error"
                  :loading="loadingBtn"
                  @click="closeTFA"
